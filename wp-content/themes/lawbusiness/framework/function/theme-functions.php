...
function register_js_scripts() {
	if (!is_admin()) {
		...
		wp_register_script('inview', get_template_directory_uri() . '/js/jquery.inview.js', array('jquery'), '1.0.0', true);
...
function the_widgets_init() {
    ...
    register_sidebar(
        array(
            ...
            'after_widget' => '</aside>', 
            'before_title' => '<h3 class="widgettitle"><span class="widgeticon"></span>', 
            'after_title' => '</h3>'
        )
    );
...
if (function_exists('add_theme_support')) {
    ...
    set_post_thumbnail_size(721, 407, true);
	
}

if (function_exists('add_image_size')) {
	add_image_size('testim-thumb', 45, 45, true);
	add_image_size('service-thumb', 470, 303, true);
	add_image_size('person-thumb', 470, 376, true);
	add_image_size('service-thumb-half', 570, 367, true);
	add_image_size('service-thumb-full', 1160, 748, true);
	add_image_size('slider-thumb', 710, 9999);
	add_image_size('full-thumb', 1040, 585, true);
	add_image_size('full-slider-thumb', 1040, 9999);
}
...
function cmsms_header_styles() {
	...
	$_header_height_calc = $cmsms_option[CMSMS_SHORTNAME . '_header_height'] - 2;

	echo '#header .header_wrap { ' . 
			'height : ' . $cmsms_option[CMSMS_SHORTNAME . '_header_height'] . 'px; ' . 
		'} ' . 
		'#header .navi_scrolled_resize .header_wrap {height:auto;}' . 
		'.header_scrolled {' . 
			'padding-bottom: ' . $cmsms_option[CMSMS_SHORTNAME . '_header_height'] . 'px;' . 
		'}' . 
		'#header a.logo { ' . 
			'margin : ' . $cmsms_option[CMSMS_SHORTNAME . '_logo_top'] . 'px ' . $cmsms_option[CMSMS_SHORTNAME . '_logo_left'] . 'px 10px; ' . 
		'} ' . 
		'.container .custom_header {' . $cmsms_option[CMSMS_SHORTNAME . '_header_social'] . 
		'}' .
		'#header a.logo, ' . 
		'#header a.logo > img { ' . 
		...
		'.custom_header { ' . 
			...
			((is_rtl()) ? 'left' : 'right') . ' : ' . $cmsms_option[CMSMS_SHORTNAME . '_header_custom_html_right'] . 'px; ' . 
		'} ' . 
		'#navigation ul,
		 #navigation > li.current-menu-item:before, 
		 #navigation > li.current-menu-parent:before, 
		 #navigation > li.current-menu-ancestor:before { ' . 
			'top : ' . $_header_height_calc . 'px; ' . 
		'} ' . 
		'#header a.logo,
		 #navigation > li { ' . 
		    'line-height : ' . $cmsms_option[CMSMS_SHORTNAME . '_header_height'] . 'px; ' . 
		'} ' . 
