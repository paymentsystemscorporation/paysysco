	...
	function insertShortcode() { 
		if (window.tinyMCE) {
			var shortcode_tag = '', 
				...
				cmsms_stats_type = jQuery('#TB_ajaxContent #stats_type_val').val(), 
			...
			for (var i = 0, ilength = cmsms_stats_bar.length; i < ilength; i += 3) {
				shortcode_tag += '[stats_bar stats_type="' + cmsms_stats_type + '" title="' + cmsms_stats_bar.eq(i).val() + '" value="' + cmsms_stats_bar.eq(i + 1).val() + '" color="' + cmsms_stats_bar.eq(i + 2).val() + '"] ';
			}
...
<div class="popup_content">
	...
	<select name="stats_type_val" id="stats_type_val" aria-required="true" class="popup_tr_value">
		<option value="round"<?php echo ($content != '' && $stats['stats_type'] && $stats['stats_type'] == 'round') ? ' selected="selected"' : ''; ?>><?php _e('Round', 'cmsmasters'); ?>&nbsp;</option>
		<option value="linear"<?php echo ($content != '' && $stats['stats_type'] && $stats['stats_type'] == 'linear') ? ' selected="selected"' : ''; ?>><?php _e('Linear', 'cmsmasters'); ?>&nbsp;</option>
	</select>
