...
function cmsmsLike($show = true) {
    ...
    if (!isset($_COOKIE['like-' . $post_ID]) && $ipCheck == 0) {
		$counter = '<a href="#" onclick="cmsmsLike(' . $post_ID . '); return false;" id="cmsmsLike-' . $post_ID . '" class="cmsmsLike"><span class="cmsms_like_inner">' . $likes . '</span></a>';
    } else {
		$counter = '<a href="#" onclick="return false;" id="cmsmsLike-' . $post_ID . '" class="cmsmsLike active"><span class="cmsms_like_inner">' . $likes . '</span></a>';
    }
